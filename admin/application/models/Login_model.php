...
class login_model extends CI_Model
{
    ...
    function get_user_details($usr, $pwd) {
        ...
        $result = 0;
        $query=$this->db->query("SELECT * FROM group_users WHERE gu_email = '$usr' and is_admin = '1' order by gu_id")->result();
        if(count($query)>0){
    ...
    function get_user_details_for_admin($usr, $pwd) {
        // $pwd = md5($pwd);
        // $query=$this->db->query("SELECT * FROM group_users WHERE gu_email = '$usr' AND gu_password = '$pwd' order by gu_gid desc");
        // return $query;
        ...
        // $query=$this->db2->query("SELECT * FROM group_users WHERE gu_email = '$usr' and assigned_role = '0' order by gu_id")->result();
