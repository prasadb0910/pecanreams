...
function purchaseData($status='', $property_id=''){
    ...
    if (count($result)>0) {
        $sql="select * from 
            (select C.txn_id, C.txn_id as property_id, C.gp_id, C.p_property_name, C.p_display_name, C.p_purchase_date, C.p_type, C.p_status, 
            ...
            from purchase_txn where gp_id='$gid'" . $cond2 . " order by modified_date desc) A 
            left join 
            ...
            where A.pr_client_id=B.ow_id and A.ow_id in (select min(ow_id) from purchase_ownership_details 
    ...
    if (count($result)>0) {
        $sql="select * from 
            ...
            (select A.txn_id, A.gp_id, A.p_property_name, A.p_display_name, A.p_purchase_date, A.p_type, A.p_status, 
                A.txn_status, B.purchase_price from 
            (select txn_id, gp_id, p_property_name, p_display_name, p_purchase_date, p_type, p_status, txn_status 
            from purchase_txn where gp_id='$gid') A 
            left join 
            (select purchase_id,sum(net_amount) as purchase_price from purchase_schedule where status = '1' or status = '3' group by purchase_id) B 
            on A.txn_id = B.purchase_id) C 
            left join 
