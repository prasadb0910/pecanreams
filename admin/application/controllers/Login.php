...
class Login extends CI_Controller
{
    ...
    public function get_user_details_for_admin(){
        // $uname='admin@admin.com';
        $uname='admin@pecanreams.com';
        $upass='pass@123';
        $result = $this->login_model->get_user_details_for_admin($uname, $upass);
        echo $result;
    }
    ...
    public function checkcredentials() {
        ...
        if ($uname=='admin@pecanreams.com' && $upass=='delta003') {
            ...
            if($module=='Assure'){
                ...
                $query = $this->db->query("SELECT * FROM group_users WHERE gu_email = '$uname' order by gu_id");
                $data = $query->result();
                if(count($data)>0){
                    $gu_id = $data[0]->gu_id;
                    $name = $data[0]->name;
        ...
        if($module=='Assure' && $authorised == true){
            $redirect_url=base_url().'index.php/login/assure';
        }
    ...
    public function check_login_details() {
        ...
        // $username = html_escape('admin@pecanreams.com');
        // $password = html_escape('pass@123');
        ...
        if ($username=='admin@pecanreams.com' && $password=='delta003') {
            ...
            $result = $this->login_model->get_user_details_for_admin($username, $password);
    ...
    public function assure() {
        ...
        redirect(base_url().'assure/public/index.php/login/get_ci_session/'.$token, 'refresh');
    }
    ...
    public function get_laravel_session($token){
        ...
        if(count($data)>0){
            ...
            $sql = "select * from group_users where user_id = '$user_id'";
            $query = $this->db->query($sql);
            $data2 = $query->result();
            if(count($data2)>0){
                $gu_id=$data2[0]->gu_id;
