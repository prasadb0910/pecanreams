...
class Loan extends CI_Controller
{
    ...
    public function saverecord(){
        ...
        if(count($result)>0) {
            ...
            // $do_type = 'property_loan';

            // $filePath='uploads/property_loan/'.$do_type.'_'.$lid.'/property_loan_documents/';
            ...
            //             $data = array(
            //                 'ln_doc_loanid' => $lid,
            //                 'ln_doc_name' => $docname[$k],
            //                 'ln_doc_description' => $docdesc[$k],
            //                 'ln_doc_ref_no' => $docref[$k],
            //                 'ln_doc_doi' => $doi,
            //                 'ln_doc_doe' => $doe,
            //                 'ln_document' => $filePath.$fileName,
            //                 'ln_document_name' => $fileName,
            //             );
            //             $this->db->insert('loan_document_details', $data);
            //             echo "Main<br>";
            //         } else {
            //             echo "Other<br>";
            //             $data = array(
    ...
    public function view($lid){
        ...
        $query=$this->db->query("SELECT * FROM user_role_options WHERE section = 'Loan' AND role_id='$roleid' AND (r_insert = 1 OR r_approvals = 1)");
        $result=$query->result();
        if(count($result)>0) {
            $data['access']=$result;
            $ptype = '';
            ...
            $sql="select F.txn_id, F.gp_id, F.ref_id, F.ref_name, F.loan_amount, F.loan_term, F.loan_interest_rate, 
                        F.loan_startdate, F.loan_due_day, F.interest_type, F.loan_type, F.financial_institution, F.repayment, F.purpose, 
                        F.txn_status, F.txn_remarks, F.created_by, F.create_date, F.modified_by, F.modified_date, 
                        F.approved_by, F.approved_date, F.loan_id, F.brower_id, F.owner_name, F.maker_remark,
                        G.property_id, G.sub_property_id, G.property, G.p_property_name, G.p_display_name, G.p_type, G.sp_name from 
                (select * from 
                (select C.txn_id, C.gp_id, C.ref_id, C.ref_name, C.loan_amount, C.loan_term, C.loan_interest_rate, 
                        C.loan_startdate, C.loan_due_day, C.interest_type, C.loan_type, C.financial_institution, C.repayment, C.purpose, 
                        C.txn_status, C.txn_remarks, C.created_by, C.create_date, C.modified_by, C.modified_date, 
                        C.approved_by, C.approved_date, D.loan_id, D.brower_id, D.owner_name, C.maker_remark from 
                (select * from loan_txn where gp_id='$gid' and txn_id='$lid') C 
                left join 
                (select A.loan_id, A.brower_id, 
                ...
                where A.brower_id=B.ow_id and A.brower_id in (select min(brower_id) from loan_borrower_details 
                where loan_id=A.loan_id group by loan_id)) D 
            ...
            $query=$this->db->query("select C.loan_id, C.property_id, C.sub_property_id, C.property, C.p_property_name, C.p_display_name, D.sp_name from 
            ...
            // $query=$this->db->query("SELECT * FROM loan_document_details WHERE ln_doc_loanid='$lid'");
            // $result=$query->result();
            // $data['editdocs']=$result;
            
            $query=$this->db->query("select C.*, D.d_documentname as doc_documentname, d_show_expiry_date from 
                                    (select A.*, B.d_type_id, B.d_type, B.d_m_status from 
                                    (select * from document_details where doc_ref_id='$lid' and doc_ref_type='Property_Loan') A 
                                    left join 
                                    (select * from document_type_master) B 
                                    on (A.doc_type_id=B.d_type_id)) C 
                                    left join 
    ...
    public function edit($lid){
        ...
        if(count($result)>0) {
            ...
            //     $data['p_docs']=$result;
            //     // for ($i=0; $i < count($result) ; $i++) { 
            //     //     if($result[$i]->ln_document!= '') {
            //     //         $this->downloadloandocs($lid, $result[$i]->ln_doc_id);
            //     //     }       
            //     // }
            // }
            ...
            // $query=$this->db->query("SELECT * FROM loan_schedule WHERE loan_id='$lid'");
            // $result=$query->result();
            // $data['editschedule']=$result;


            // $data['sub_property']= $this->loan_model->getSubPropertyDetails($lid, $property_id);
            ...
            if($txn_status=='Delete') {
                ...
                if($result[0]->r_edit==1) {
                    ...
                    // if ($txn_status!="Delete" || $rec_status=="Approved") {
                        ...
                        // for ($i=0; $i < count($result) ; $i++) { 
                        //     $file_path_db[$i]['docname']=$result[$i]->ln_doc_name;
                        //     $file_path_db[$i]['docdesc']=$result[$i]->ln_doc_description;
                        //     $file_path_db[$i]['docrefno']=$result[$i]->ln_doc_ref_no;
                        //     $file_path_db[$i]['docdoi']=$result[$i]->ln_doc_doi;
                        //     $file_path_db[$i]['docdoe']=$result[$i]->ln_doc_doe;
                        //     $file_path_db[$i]['docpath']=$result[$i]->ln_document;
                        //     $file_path_db[$i]['docfilename']=$result[$i]->ln_document_name;
                        //     $file_path_count=$i;
                        // }
    ...
    public function checkstatus($status='') {
        ...
        if(count($result)>0) {
            $data['access']=$result;
            $data['loan']=$this->loan_model->loanData($status);
