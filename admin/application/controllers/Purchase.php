...
class Purchase extends CI_Controller
{
    ...
    public function saverecord(){
        ...
        if(count($result)>0) {
            ...
            $filePath='uploads/property_purchase/'.$do_type.'_'.$pid.'/property_purchase_images/';
            ...
            // $filePath='uploads/property_purchase/'.$do_type.'_'.$pid.'/property_purchase_documents/';
            // $upload_path = './' . $filePath;
            // if(!is_dir($upload_path)) {
            //     mkdir($upload_path, 0777, TRUE);
            // }
            ...
            //             $data = array(
            //                 'pr_doc_purchaseid' => $pid,
            //                 'pr_doc_name' => $docname[$k],
            //                 'pr_doc_description' => $docdesc[$k],
            //                 'pr_doc_ref_no' => $docref[$k],
            //                 'pr_doc_doi' => $doi,
            //                 'pr_doc_doe' => $doe,
            //                 'pr_document' => $filePath.$fileName,
            //                 'pr_document_name' => $fileName,
            //                 'fk_d_id' => $docid[$k]
            //             );
            //             $this->db->insert('purchase_document_details', $data);
            //             echo "Main<br>";
            //         } else {
            //             echo "Other<br>";
            //             $data = array(
    ...
    public function edit($pid){
        ...
        if(count($result)>0) {
            if($result[0]->r_edit == 1 or $result[0]->r_approvals == 1) {
                ...
                // $query=$this->db->query("SELECT A.*, B.d_m_status FROM (SELECT * FROM purchase_document_details WHERE pr_doc_purchaseid = '$pid') A LEFT JOIN (SELECT * FROM document_master) B ON (A.fk_d_id=B.d_id)");
                // $result=$query->result();
                // if(count($result)>0){
                //     $data['p_docs']=$result;
                // }
            ...
            if($result[0]->r_edit == 1 or $result[0]->r_approvals == 1 or $result[0]->r_view == 1) {
                ...
                for ($i=0; $i < count($result) ; $i++) { 
                    if($result[$i]->ow_type==0) {
                        ...
                        $quer=$this->db->query("SELECT * FROM contact_master WHERE c_id = '$cd'");
                        $res=$quer->result();
                        $data['ownerdetails'][$i]=$res[0]->c_name . ' ' . $res[0]->c_last_name;
                    } else if($result[$i]->ow_type==1) {
                        $data['ownerdetails'][$i]=$result[$i]->ow_huf_name;
                    } else if($result[$i]->ow_type==2) {
                        $data['ownerdetails'][$i]=$result[$i]->ow_pvtltd_comapny_name;
            ...
            if($txn_status=='Delete') {
                ...
                if($result[0]->r_edit == 1) {
                    ...
                    $data = array(
                        ...
                        'p_display_name' => $this->input->post('display_name'),
                    ...
                    $this->db->where('purchase_id', $pid);
                    $this->db->update('purchase_property_description', $data);
                    ...
                    // $query=$this->db->query("SELECT * FROM purchase_document_details WHERE pr_doc_purchaseid = '$pid'");
                    // $result=$query->result();
                    // $file_path_db=NULL;
                    // $file_path_count=0;

                    // for ($i=0; $i < count($result) ; $i++) { 
                    //     $file_path_db[$i]['docname']=$result[$i]->pr_doc_name;
                    //     $file_path_db[$i]['docdesc']=$result[$i]->pr_doc_description;
                    //     $file_path_db[$i]['docrefno']=$result[$i]->pr_doc_ref_no;
                    //     $file_path_db[$i]['docdoi']=$result[$i]->pr_doc_doi;
                    //     $file_path_db[$i]['docdoe']=$result[$i]->pr_doc_doe;
                    //     $file_path_db[$i]['docpath']=$result[$i]->pr_document;
                    //     $file_path_db[$i]['docfilename']=$result[$i]->pr_document_name;
                    ...
                    //                 'pr_doc_doi' => $doi,
                    //                 'pr_doc_doe' => $doe,
                    //                 // 'pr_document' => $destination,
                    //                 // 'pr_document_name' => $docname[$k].$extension,
                    //                 'pr_document' => $filePath.$fileName,
                    //                 'pr_document_name' => $fileName,
                    //                 'fk_d_id' => $docid[$k]
                    //             );
