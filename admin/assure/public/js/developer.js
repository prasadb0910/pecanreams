...
var set_total_fsi_donut = function (sold, unsold) {
 ...
 //    var donutData = [
 //     {label: "Approved", data: 30, color: "#004d99"},
 //      {label: "Proposed", data: 20, color: "rgb(204, 0, 0)"},
 //    ];
 //    $.plot("#total_fsi", donutData, {
 //      series: {
 //        pie: {
 //          show: true,
 //          radius: 1,
...
$('#btn_get_details').click(function(){
	var developers = $('#developer').val();
	var project_types = $('#project_type').val();
    var entity_type = $('#entity_type').val();
	// if (developers!=null || prop_types!=null || entity_type!=null){
	// 	get_data(developers, prop_types, entity_type);
	// }
    get_data(developers, project_types, entity_type);
});

var get_data = function(developers, prop_types, entity_type){
	var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');

    $.ajax({
        url:BASE_URL+'developer/get_data',
        type:'post',
        data: {_token: CSRF_TOKEN, developers: developers, prop_types: prop_types, entity_type: entity_type},
        datatype:'json',
        beforeSend: function(){
            $('.chart_div').html('');
            $('.loader').show();
