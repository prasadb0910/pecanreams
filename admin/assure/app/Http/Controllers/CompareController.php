...
class CompareController extends Controller
{
    ...
    public function index() {
        ...
        $sql = "select distinct developer from 
        ...
        $developer = DB::select($sql);
        $data['developer'] = $developer;

        $sql = "select distinct master_project_name from project_details where status ='approved' order by master_project_name";
        $master_project_name = DB::select($sql);
        $data['master_project_name'] = $master_project_name;
    ...
    public function get_data(Request $request){
        ...
        if(isset($input['projects'])){
            ...
            if(isset($projects)){
                $cond = $cond . " and master_project_name in (" . $projects . ")";
            }
        }
        ...
        $sql = "select sub_location, sum(total_carpet_area) as total_carpet_area, 
                    sum(sold_carpet_area) as sold_carpet_area, 
                    sum(unsold_carpet_area) as unsold_carpet_area, 
                    sum(case when apartment_type_updated = 'Others' then 0 else number_of_apartment end) as total_units, 
                    sum(case when apartment_type_updated = 'Others' then 0 else no_of_booked_apartment end) as sold_units, 
