...
function insertDetails($form_data) {
	...
	for($i=0; $i<count($follower); $i++) {
		...
		if(count($result)>0){
			$follower_name="";
	        if(isset($result[0]->c_name)){
	            $follower_name=$result[0]->c_name;
	        }
	        if(isset($result[0]->c_last_name)){
	            $follower_name=$follower_name.' '.$result[0]->c_last_name;
	        }
			$follower_email=$result[0]->c_emailid1;
		    ...
		    $message = '<html><head></head><body>Dear '.$follower_name.'<br /><br />
		                ...
		                The Task details are as follows.<br /><br />' . $table . '<br /><br />
		                If the above Task is incorrectly assigned to you please reject the same immediately.<br /><br />Thanks</body></html>';
		    $mailSent=send_email($from_email,  $from_email_sender, $follower_email, $subject, $message);
		    // print_r($mailSent);
		}
	}
...
function getTaskList($user_id,$task_type){
    ...
    if (count($result)>0) {
    	$sql="select G.*, datediff(G.due_date,str_to_date(CURDATE(),'%Y-%m-%d')) AS no_of_days, 
			...
			where user_id = '$user_id') group by purchase_id)) H 
    	...
    	$sql="select G.*, datediff(G.due_date,str_to_date(CURDATE(),'%Y-%m-%d')) AS no_of_days, 
			...
			from purchase_ownership_details A, owner_master B 
			where A.pr_client_id=B.ow_id and A.pr_client_id in (select min(pr_client_id) from purchase_ownership_details 
			where purchase_id=A.purchase_id group by purchase_id)) H 
			on G.property_id = H.purchase_id";
    }
    $query=$this->db->query($sql);
    return $query->result();
