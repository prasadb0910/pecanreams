...
class Loan_model Extends CI_Model{
    ...
    function loanData($status='', $property_id='', $lid=''){
        ...
        if (count($result)>0) {
            $blOwnerExist = true;
            $sql="select * from 
                (select F.txn_id, F.gp_id, F.ref_id, F.ref_name, F.loan_amount, F.loan_term, F.loan_interest_rate, 
                ...
                where user_id = '$session_id') group by loan_id)) D 
    ...
    function getPropertyDetails($term = "t") {
        $gid=$this->session->userdata('groupid');
        $roleid=$this->session->userdata('role_id');
        $session_id=$this->session->userdata('session_id');
        ...
        if (count($result)>0) {
            $sql="select * from 
                ...
                where sales_id is null) H where H.p_property_name like '%" . $term . "%'";
        } else {
            $sql="select * from 
                (select distinct txn_id, p_property_name, p_display_name from 
                (select E.txn_id, E.p_property_name, E.p_display_name, E.p_purchase_date, E.sp_id, E.sp_name, 
