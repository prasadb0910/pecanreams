...
class Bank extends CI_Controller
{
    ...
    public function addnew(){
        ...
        if(count($result)>0) {
            ...
            // $data['owner_name']=array();
            // for ($i=0; $i < count($result) ; $i++) { 
            //     if($result[$i]->ow_type==0) {
            //         $coid=$result[$i]->ow_ind_id;
            //         $query1=$this->db->query("SELECT * FROM contact_master WHERE c_id = '$coid'");
            //         $result1=$query1->result();
            //         if(count($result1)>0){
            //             $data['owner_name'][$i]=$result1[0]->c_name;
            //         } else {
            //             $data['owner_name'][$i]='';
            //         }
            //     } else if ($result[$i]->ow_type==1) {
            //         $data['owner_name'][$i]=$result[$i]->ow_huf_name;
            //     } else if ($result[$i]->ow_type==2) {
            //         $data['owner_name'][$i]=$result[$i]->ow_pvtltd_comapny_name;
            //     } else if ($result[$i]->ow_type==3) {
            //         $data['owner_name'][$i]=$result[$i]->ow_ltd_comapny_name;
            //     } else if ($result[$i]->ow_type==4) {
            //         $data['owner_name'][$i]=$result[$i]->ow_llp_comapny_name;
            //     } else if ($result[$i]->ow_type==5) {
            //         $data['owner_name'][$i]=$result[$i]->ow_prt_comapny_name;
            //     } else if ($result[$i]->ow_type==6) {
            //         $data['owner_name'][$i]=$result[$i]->ow_aop_comapny_name;
            //     } else if ($result[$i]->ow_type==7) {
            //         $data['owner_name'][$i]=$result[$i]->ow_trs_comapny_name;
            //     } else if ($result[$i]->ow_type==8) {
            //         $data['owner_name'][$i]=$result[$i]->ow_proprietorship_comapny_name;
            //     } else {
            //         $data['owner_name']="Try";
            //     }
            // }
    ...
    public function viewbank($bid) {
        ...
        if(count($result)>0) {
            if($result[0]->r_edit==1  or $result[0]->r_approvals==1 or $result[0]->r_view == 1) {
                ...
                // $query=$this->db->query("SELECT A.b_id, A.b_ownerid, case when B.ow_type = '0' 
                //                         then (select concat(ifnull(c_name,''),' ',ifnull(c_last_name,'')) as c_name 
                ...
                //                         A.registered_address, A.registered_phone, A.maker_remark, A.registered_email, 
                //                         A.b_name, A.b_branch, A.b_address, A.b_landmark, A.b_city, A.b_pincode, A.b_state, 
                //                         A.b_country, A.b_accounttype, A.b_accountnumber, 
                //                         A.b_customerid, A.b_ifsc, A.b_micr, A.b_relationshipmanager, 
                //                         (select concat(ifnull(c_name,''),' ',ifnull(c_last_name,'')) as c_name 
                //                             from contact_master where c_id = A.b_relationshipmanager) as c_name, 
                //                         A.b_phone_number, A.b_openingbalance, A.b_bal_ref_date, A.b_status, A.txn_remarks, 
                //                         A.create_date, A.created_by, 
                //                         A.modified_date, A.modified_by, A.approved_date, A.approved_by, A.b_gid 
                //                         FROM bank_master A, owner_master B 
                //                         WHERE A.b_id = '$bid' and A.b_ownerid=B.ow_id");

                
                $query=$this->db->query("SELECT A.*, B.owner_name, 
                                            (select concat(ifnull(c_name,''),' ',ifnull(c_last_name,'')) as c_name 
                                            from contact_master where c_id = A.b_relationshipmanager) as c_name FROM 
                                        (SELECT * FROM bank_master WHERE b_id = '$bid') A 
                                        LEFT JOIN 
                                        (select concat('c_',c_id) as ow_id, contact_name as owner_name from 
                                        ...
                                        union all 
                                        select concat('o_',ow_id) as ow_id, owner_name as owner_name from 
                ...
                $query=$this->db->query("SELECT A.ath_id, A.ath_bnk_id, A.ath_contactid, A.ath_purpose, A.ath_type, concat(ifnull(B.c_name,''),' ',ifnull(B.c_last_name,'')) as c_name FROM bank_authorizedsignatory A, contact_master B WHERE A.ath_bnk_id = '$bid' and A.ath_contactid=B.c_id");
                $result=$query->result();
                ...
                $sql="select sum(tot_amount_paid) as tot_amount_paid from 
                    ...
                    select sum(expense_amount) as tot_amount_paid from actual_other_expense where txn_status = 'Approved' and 
                        account_number = '$bid' and payment_date > '$b_bal_ref_date' 
                    union all 
                    select sum(amount) as tot_amount_paid from actual_other_schedule where txn_status = 'Approved' and 
                ...
                $sql="select sum(tot_amount_received) as tot_amount_received from 
                    ...
                    union all 
                    select sum(amount) as tot_amount_received from actual_other_schedule where txn_status = 'Approved' and 
    ...
    public function editbank($bid) {
        ...
        if(count($result)>0) {
            if($result[0]->r_edit==1  or $result[0]->r_approvals==1 ) {
                ...
                // $query=$this->db->query("SELECT A.b_id, A.b_ownerid, case when B.ow_type = '0' then 
                //                         (select concat(ifnull(c_name,''),' ',ifnull(c_last_name,'')) as c_name from contact_master where c_id = B.ow_ind_id) 
                //                         when B.ow_type = '1' then B.ow_huf_name when B.ow_type = '2' then B.ow_pvtltd_comapny_name 
                ...
                //                         when B.ow_type = '7' then B.ow_trs_comapny_name else B.ow_proprietorship_comapny_name end as owner_name, 
                //                         A.registered_address, A.registered_phone, A.registered_email, A.b_name, A.b_branch, 
                //                         A.b_address, A.b_landmark, A.b_city, A.b_pincode, A.b_state, A.b_country, 
                //                         A.b_accounttype, A.b_accountnumber, A.b_customerid, 
                //                         A.b_ifsc, A.b_micr, A.b_relationshipmanager, A.maker_remark,
                //                         (select concat(ifnull(c_name,''),' ',ifnull(c_last_name,'')) as c_name from contact_master where c_id = A.b_relationshipmanager) as c_name, 
                //                         A.b_phone_number, A.b_openingbalance, A.b_bal_ref_date, A.b_status, A.txn_remarks, 
                //                         A.create_date, A.created_by, A.modified_date, A.modified_by, A.approved_date, A.approved_by, 
                //                         A.b_gid FROM bank_master A, owner_master B WHERE A.b_id = '$bid' and A.b_ownerid=B.ow_id");
    ...
    public function getBankData($status='', $bid=''){
        ...
        for ($i=0; $i < count($result) ; $i++) { 
            ...
            $quer=$this->db->query("select * from 
                                    (select concat('c_',c_id) as ow_id, contact_name as owner_name, c_id as owner_contact from 
                                    (select A.*, concat(ifnull(A.c_name,''),' ',ifnull(A.c_last_name,''),' - ',ifnull(A.c_emailid1,''),' - ',
                                        ifnull(A.c_mobile1,''),' - ',ifnull(A.c_company,''),' - ',ifnull(B.contact_type,'')) as contact_name from 
                                    (select * from contact_master where c_status='Approved' and c_gid='$gid' and c_type='Others') A 
                                    left join 
                                    ...
                                    on (A.c_contact_type = B.id)) C 
                                    union all 
                                    select concat('o_',ow_id) as ow_id, owner_name as owner_name, owner_contact from 
                                    (select ow_gid, ow_id, ow_type, case when ow_type = '0' then ow_ind_name 
                                        when ow_type = '1' then ow_huf_name when ow_type = '2' then ow_pvtltd_comapny_name 
                                        when ow_type = '3' then ow_ltd_comapny_name when ow_type = '4' then ow_llp_comapny_name 
                                        when ow_type = '5' then ow_prt_comapny_name when ow_type = '6' then ow_aop_comapny_name 
                                        when ow_type = '7' then ow_trs_comapny_name else ow_proprietorship_comapny_name end as owner_name, 
                                        case when ow_type = '0' then ow_ind_id 
                                        when ow_type = '1' then ow_huf_karta_id when ow_type = '2' then ow_pvtltd_contact 
                                        when ow_type = '3' then ow_ltd_contact when ow_type = '4' then ow_llp_contact 
                                        when ow_type = '5' then ow_prt_contact when ow_type = '6' then ow_aop_contact 
                                        when ow_type = '7' then ow_trs_contact else ow_proprietorship_contact end as owner_contact
                                        from (select ow_gid, ow_id, ow_type, 
                                            (select concat(ifnull(c_name,''),' ',ifnull(c_last_name,'')) as c_name from contact_master 
                                                where c_id = ow_ind_id) as ow_ind_name, ow_huf_name, 
                                            ow_pvtltd_comapny_name, ow_ltd_comapny_name, ow_llp_comapny_name, ow_prt_comapny_name, 
                                            ow_aop_comapny_name, ow_trs_comapny_name, ow_proprietorship_comapny_name, 
                                            ow_ind_id, ow_huf_karta_id, ow_pvtltd_contact, ow_ltd_contact, ow_llp_contact, 
                                            ow_prt_contact, ow_aop_contact, ow_trs_contact, ow_proprietorship_contact from owner_master 
                                    where ow_status='Approved' and ow_gid='$gid') A) B 
                                    where ow_gid='$gid') AA 
                                    where AA.ow_id='$owid'");
            $result1=$quer->result();
            ...
            if(count($result1)>0){
                $data['bankowner'][$i]['name']=$result1[0]->owner_name;
                $contact_id=$result1[0]->owner_contact;
            } else {
                $data['bankowner'][$i]['name']='';
                $contact_id='';
            }

            // if($result1[0]->ow_type==0) {
            //     $coid=$result1[0]->ow_ind_id;
            //     $query=$this->db->query("SELECT * FROM contact_master WHERE c_id = '$coid';");
            //     $result2=$query->result();
            //     if(count($result2)>0){
            //         $data['bankowner'][$i]['name']=$result2[0]->c_name . ' ' . $result2[0]->c_last_name;
            //     } else {
            //         $data['bankowner'][$i]['name']='';
            //     }
            //     $contact_id=$result1[0]->ow_ind_id;
            // } else if ($result1[0]->ow_type==1) {
            //     $data['bankowner'][$i]['name']=$result1[0]->ow_huf_name;
            //     $contact_id=$result1[0]->ow_huf_karta_id;
            // } else if ($result1[0]->ow_type==2) {
            //     $data['bankowner'][$i]['name']=$result1[0]->ow_pvtltd_comapny_name;
            //     $contact_id=$result1[0]->ow_pvtltd_contact;
            // } else if ($result1[0]->ow_type==3) {
            //     $data['bankowner'][$i]['name']=$result1[0]->ow_ltd_comapny_name;
            //     $contact_id=$result1[0]->ow_ltd_contact;
            // } else if ($result1[0]->ow_type==4) {
            //     $data['bankowner'][$i]['name']=$result1[0]->ow_llp_comapny_name;
            //     $contact_id=$result1[0]->ow_llp_contact;
            // } else if ($result1[0]->ow_type==5) {
            //     $data['bankowner'][$i]['name']=$result1[0]->ow_prt_comapny_name;
            //     $contact_id=$result1[0]->ow_prt_contact;
            // } else if ($result1[0]->ow_type==6) {
            //     $data['bankowner'][$i]['name']=$result1[0]->ow_aop_comapny_name;
            //     $contact_id=$result1[0]->ow_aop_contact;
            // } else if ($result1[0]->ow_type==7) {
            //     $data['bankowner'][$i]['name']=$result1[0]->ow_trs_comapny_name;
            //     $contact_id=$result1[0]->ow_trs_contact;
            // } else if ($result1[0]->ow_type==8) {
            //     $data['bankowner'][$i]['name']=$result1[0]->ow_proprietorship_comapny_name;
            //     $contact_id=$result1[0]->ow_proprietorship_contact;
            // } else {
            //     $contact_id="";
            // }
            
            $rmid=$result[$i]->b_relationshipmanager;
            $quer=$this->db->query("SELECT * FROM contact_master WHERE c_id = '$rmid'");
            $result2=$quer->result();
            if(count($result2)>0){
                $data['relationshipmanager'][$i]=$result2[0]->c_name . ' ' . $result2[0]->c_last_name;
            } else {
                $data['relationshipmanager'][$i]='';
            }

            $quer=$this->db->query("SELECT * FROM contact_master WHERE c_id = '$contact_id'");
            $result2=$quer->result();
            if(count($result2)>0){
                $data['bankowner'][$i]['email']=$result2[0]->c_emailid1;
            } else {
