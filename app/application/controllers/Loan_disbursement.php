...
class Loan_disbursement extends CI_Controller
{
    ...
    public function view($lid){
        ...
        if(count($result)>0) {
            ...
            $sql="select E.*, F.ref_id as loan_ref_id, F.ref_name as loan_ref_name, F.loan_type, F.loan_amount, 
                  ...
                  F.repayment, F.purpose, F.financial_institution from 
           ...
           $sql="SELECT * FROM loan_schedule  WHERE loan_disbursement_id = '".$lid."' and status = '$txn_status' ";
            ...
            load_view('loan/loan_disbursement_view',$data);
        } else {
            echo '<script>alert("You donot have access to this page.");</script>';
            $this->load->view('login/main_page');
        }
    ...
    public function checkstatus($status='') {
        ...
        if(count($result)>0) {
            $data['access']=$result;
            $data['loan']=$this->loan_disbursement_model->loanData($status);

            $count_data=$this->loan_disbursement_model->loanData();
            $approved=0;
            $pending=0;
            $rejected=0;
            $inprocess=0;
